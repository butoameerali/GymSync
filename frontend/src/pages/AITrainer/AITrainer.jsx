          ...
          <div className="ai-plan-view glass-panel">
            <div style={{display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px'}}>
              ...
              <div>
                ...
                <p style={{color: 'var(--text-secondary)'}}>Generated based on your Profile Bio Goals</p>
              </div>
            </div>
            
            <div className="exercise-grid">
              {/* Mock AI picking 3 exercises based on goals */}
              {EXERCISE_LIBRARY.slice(0, 3).map(ex => (
                <div key={ex.id} className="exercise-card" style={{border: '1px solid rgba(59, 130, 246, 0.3)'}}>
                  <div className="ex-card-header">
                    <h4>{ex.name}</h4>
                    <span style={{fontSize: '0.8rem', color: '#3b82f6'}}>Today's Plan</span>
                  </div>
                  <span className="ex-category">{ex.category}</span>
                  <button className="btn btn-primary btn-sm w-100 mt-10" onClick={() => startExercise(ex)}>Start Target</button>
                </div>
              ))}
            </div>
          </div>
